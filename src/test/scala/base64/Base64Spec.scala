...
class Base64Spec extends FunSpec {
  describe ("Encode") {
    ...
    it("should encode with and without padding") {
      val str = "easure."
      def check(pad: Boolean, expect: Array[Byte]): Assertion = {
        val enc = Encode(str, pad = pad)
        assert(enc === expect)
        assert(Decode(enc).right.map(new String(_)) === Right(str))
      }
      check(pad = true, expect = "ZWFzdXJlLg==".getBytes)
      check(pad = false, expect = "ZWFzdXJlLg".getBytes)
    }
  ...
  describe ("Decode") {
    it("should not bother decoding pad only input") {
      ("====" :: "===" :: "==" :: "=" :: Nil).foreach {
        p => assert(Decode(p).right.map(_.length) === Right(0))
      }
    }
